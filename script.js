const config = {
  symbols: ["O", "X", "*", ">", "$", "W"],
  blockSize: 25,
  detectionRadius: 50, // maximum distance to detect block(no detection for far than that value)
  clusterSize: 7, // how many block will be effected at 1 trigger
  blockLifetime: 300, // animation time
  emptyRatio: 0.3, // random ratio of being empty
  scrambleRatio: 0.25, // keep changing symbol per time
...
function initGridOverlay(element) {
  ...
  element.addEventListener("mousemove", (e) => {
    ...
    // if block is marked as scramble and its not started scramble yet
    ...
    for (let i = 0; i < clusterCount; i++) {
      ...
      randomNeighbor.highlightEndTime =
        currentTime + config.blockLifetime + i * 10; // stagger
